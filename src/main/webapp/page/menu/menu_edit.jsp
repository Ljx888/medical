...
%>
        ...
        <form method="post" class="form-x" action="page/menu/edit">
        ...
        <div class="field">
        <input type="text" class="input w50" value="${menu.menuId}" name="menuId" data-validate="required:请输入标题" />
        <div class="tips"></div>
        </div>
        </div>
        <div class="form-group">
        ...
        <option value="${menu.menuPid}-${menu.level-1}">${menu.menuName} |- ${menu.url}</option>
        <option value="0">主菜单</option>
        <c:forEach items="${list}" var="list" varStatus="i">
            <c:if test="${list.level == 1}">
                <option value="${list.menuId}-${list.level}">${list.menuName} |- ${list.url}</option>
            </c:if>
